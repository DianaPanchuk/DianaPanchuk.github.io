...
// ==============
$(window).on("load",function(){$("[data-spy=\"affix\"]").each(function(){var $spy=$(this);var data=$spy.data();data.offset=data.offset||{};if(data.offsetBottom!=null)data.offset.bottom=data.offsetBottom;if(data.offsetTop!=null)data.offset.top=data.offsetTop;Plugin.call($spy,data)})})}(jQuery);/*! lightslider - v1.1.6 - 2016-10-25
 ...
 * Released under the MIT license
 */(function(factory){if(typeof define==="function"&&define.amd){define(["jquery"],factory)}else if((typeof module==="undefined"?"undefined":_typeof(module))==="object"&&module.exports){module.exports=factory(require("jquery"))}else{factory(jQuery)}})(function($){$.extend($.fn,{// http://jqueryvalidation.org/validate/
...
* Copyright (c) 2015 James Florentino; Licensed MIT */(function(factory){if(typeof define==="function"&&define.amd){return define(["jquery"],function($){return factory($,window,document)})}else if((typeof exports==="undefined"?"undefined":_typeof(exports))==="object"){return module.exports=factory(require("jquery"),window,document)}else{return factory(jQuery,window,document)}})(function($,window,document){"use strict";var BROWSER_IS_IE7,BROWSER_SCROLLBAR_WIDTH,DOMSCROLL,DOWN,DRAG,ENTER,KEYDOWN,KEYUP,MOUSEDOWN,MOUSEENTER,MOUSEMOVE,MOUSEUP,MOUSEWHEEL,NanoScroll,PANEDOWN,RESIZE,SCROLL,SCROLLBAR,TOUCHMOVE,UP,WHEEL,cAF,defaults,getBrowserScrollbarWidth,hasTransform,isFFWithBuggyScrollbar,rAF,transform,_elementStyle,_prefixStyle,_vendor;defaults={/**
   ...
   */BROWSER_IS_IE7=window.navigator.appName==="Microsoft Internet Explorer"&&/msie 7./i.test(window.navigator.appVersion)&&window.ActiveXObject;/**
    ...
    @default null
    @private
   */BROWSER_SCROLLBAR_WIDTH=null;rAF=window.requestAnimationFrame;cAF=window.cancelAnimationFrame;_elementStyle=document.createElement("div").style;_vendor=function(){var i,transform,vendor,vendors,_i,_len;vendors=["t","webkitT","MozT","msT","OT"];for(i=_i=0,_len=vendors.length;_i<_len;i=++_i){vendor=vendors[i];transform=vendors[i]+"ransform";if(transform in _elementStyle){return vendors[i].substr(0,vendors[i].length-1)}}return false}();_prefixStyle=function _prefixStyle(style){if(_vendor===false){return false}if(_vendor===""){return style}return _vendor+style.charAt(0).toUpperCase()+style.substr(1)};transform=_prefixStyle("transform");hasTransform=transform!==false;/**
    Returns browser's native scrollbar width
    @method getBrowserScrollbarWidth
    @return {Number} the scrollbar width in pixels
    @static
    @private
   */getBrowserScrollbarWidth=function getBrowserScrollbarWidth(){var outer,outerStyle,scrollbarWidth;outer=document.createElement("div");outerStyle=outer.style;outerStyle.position="absolute";outerStyle.width="100px";outerStyle.height="100px";outerStyle.overflow=SCROLL;outerStyle.top="-9999px";document.body.appendChild(outer);scrollbarWidth=outer.offsetWidth-outer.clientWidth;document.body.removeChild(outer);return scrollbarWidth};isFFWithBuggyScrollbar=function isFFWithBuggyScrollbar(){var isOSXFF,ua,version;ua=window.navigator.userAgent;isOSXFF=/(?=.+Mac OS X)(?=.+Firefox)/.test(ua);if(!isOSXFF){return false}version=/Firefox\/\d{2}\./.exec(ua);if(version){version=version[0].replace(/\D+/g,"")}return isOSXFF&&+version>23};/**
    @class NanoScroll
    @param element {HTMLElement|Node} the main element
    @param options {Object} nanoScroller's options
    @constructor
   */NanoScroll=function(){function NanoScroll(el,options){this.el=el;this.options=options;BROWSER_SCROLLBAR_WIDTH||(BROWSER_SCROLLBAR_WIDTH=getBrowserScrollbarWidth());this.$el=$(this.el);this.doc=$(this.options.documentContext||document);this.win=$(this.options.windowContext||window);this.body=this.doc.find("body");this.$content=this.$el.children("."+this.options.contentClass);this.$content.attr("tabindex",this.options.tabIndex||0);this.content=this.$content[0];this.previousPosition=0;if(this.options.iOSNativeScrolling&&this.el.style.WebkitOverflowScrolling!=null){this.nativeScrolling()}else{this.generate()}this.createEvents();this.addEvents();this.reset()}/**
     ...
     */NanoScroll.prototype.scrollBottom=function(offsetY){if(!this.isActive){return}this.$content.scrollTop(this.contentHeight-this.$content.height()-offsetY).trigger(MOUSEWHEEL);this.stop().restore();return this};/**
     ...
     */NanoScroll.prototype.scrollTop=function(offsetY){if(!this.isActive){return}this.$content.scrollTop(+offsetY).trigger(MOUSEWHEEL);this.stop().restore();return this};/**
     ...
     */NanoScroll.prototype.scrollTo=function(node){if(!this.isActive){return}this.scrollTop(this.$el.find(node).get(0).offsetTop);return this};/**
     ...
     */NanoScroll.prototype.stop=function(){if(cAF&&this.scrollRAF){cAF(this.scrollRAF);this.scrollRAF=null}this.stopped=true;this.removeEvents();if(!this.iOSNativeScrolling){this.pane.hide()}return this};/**
      Destroys nanoScroller and restores browser's native scrollbar.
      @method destroy
      @chainable
      @example
          $(".nano").nanoScroller({ destroy: true });
     */NanoScroll.prototype.destroy=function(){if(!this.stopped){this.stop()}if(!this.iOSNativeScrolling&&this.pane.length){this.pane.remove()}if(BROWSER_IS_IE7){this.$content.height("")}this.$content.removeAttr("tabindex");if(this.$el.hasClass("has-scrollbar")){this.$el.removeClass("has-scrollbar");this.$content.css({right:""})}return this};/**
     ...
     */NanoScroll.prototype.flash=function(){if(this.iOSNativeScrolling){return}if(!this.isActive){return}this.reset();this.pane.addClass("flashed");setTimeout(function(_this){return function(){_this.pane.removeClass("flashed")}}(this),this.options.flashDelay);return this};return NanoScroll}();$.fn.nanoScroller=function(settings){return this.each(function(){var options,scrollbar;if(!(scrollbar=this.nanoscroller)){options=$.extend({},defaults,settings);this.nanoscroller=scrollbar=new NanoScroll(this,options)}if(settings&&(typeof settings==="undefined"?"undefined":_typeof(settings))==="object"){$.extend(scrollbar.options,settings);if(settings.scrollBottom!=null){return scrollbar.scrollBottom(settings.scrollBottom)}if(settings.scrollTop!=null){return scrollbar.scrollTop(settings.scrollTop)}if(settings.scrollTo){return scrollbar.scrollTo(settings.scrollTo)}if(settings.scroll==="bottom"){return scrollbar.scrollBottom(0)}if(settings.scroll==="top"){return scrollbar.scrollTop(0)}if(settings.scroll&&settings.scroll instanceof $){return scrollbar.scrollTo(settings.scroll)}if(settings.stop){return scrollbar.stop()}if(settings.destroy){return scrollbar.destroy()}if(settings.flash){return scrollbar.flash()}}return scrollbar.reset()})};$.fn.nanoScroller.Constructor=NanoScroll});//# sourceMappingURL=jquery.nanoscroller.js.map
(function($){"use strict";//<editor-fold desc="Shims">
...
var valHooks={useDefault:false,_set:$.valHooks.select.set};$.valHooks.select.set=function(elem,value){if(value&&!valHooks.useDefault)$(elem).data("selected",true);return valHooks._set.apply(this,arguments)};var changed_arguments=null;$.fn.triggerNative=function(eventName){var el=this[0],event;if(el.dispatchEvent){// for modern browsers & IE9+
if(typeof Event==="function"){// For modern browsers
...
$.expr.pseudos.icontains=function(obj,index,meta){var $obj=$(obj);var haystack=($obj.data("tokens")||$obj.text()).toString().toUpperCase();return haystack.includes(meta[3].toUpperCase())};// Case insensitive begins search
$.expr.pseudos.ibegins=function(obj,index,meta){var $obj=$(obj);var haystack=($obj.data("tokens")||$obj.text()).toString().toUpperCase();return haystack.startsWith(meta[3].toUpperCase())};// Case and accent insensitive contains search
$.expr.pseudos.aicontains=function(obj,index,meta){var $obj=$(obj);var haystack=($obj.data("tokens")||$obj.data("normalizedText")||$obj.text()).toString().toUpperCase();return haystack.includes(meta[3].toUpperCase())};// Case and accent insensitive begins search
$.expr.pseudos.aibegins=function(obj,index,meta){var $obj=$(obj);var haystack=($obj.data("tokens")||$obj.data("normalizedText")||$obj.text()).toString().toUpperCase();return haystack.startsWith(meta[3].toUpperCase())};/**
...
if(this.validity.valid)that.$button.removeClass("bs-invalid");that.$element.off("rendered.bs.select")}})})}setTimeout(function(){that.$element.trigger("loaded.bs.select")})},createDropdown:function createDropdown(){// Options
...
var $opt=$(element.options[element.selectedIndex]);if($opt.attr("selected")===undefined&&this.$element.data("selected")===undefined){titleOption.selected=true}}}this.$element.find("option").each(function(index){var $this=$(this);liIndex++;if($this.hasClass("bs-title-option"))return;// Get the class and text for the option
...
if(!showDivider&&that.options.hideDisabled){// get previous elements
var $prev=$(this).prevAll();for(var i=0;i<$prev.length;i++){// find the first element in the previous elements that is an optgroup
if($prev[i].tagName==="OPTGROUP"){var optGroupDistance=0;// loop through the options in between the current option and the optgroup
// and check if they are hidden or disabled
for(var d=0;d<i;d++){var prevOption=$prev[d];if(prevOption.disabled||$(prevOption).data("hidden")===true)optGroupDistance++}// if all of the options between the current option and the optgroup are hidden or disabled, show the divider
if(optGroupDistance===i)showDivider=true;break}}}if(showDivider){liIndex++;_li.push(generateLI("",null,"divider",optID+"div"))}_li.push(generateLI(generateA(text,optionClass,inline,tokens),index))}that.liObj[index]=liIndex});//If we are not multiple, we don't have a selected item, and we don't have a title, select the first element so something is set in the button
if(!this.multiple&&this.$element.find("option:selected").length===0&&!this.options.title){this.$element.find("option").eq(0).prop("selected",true).attr("selected","selected")}return _li.join("")},findLis:function findLis(){if(this.$lis==null)this.$lis=this.$menu.find("li");return this.$lis},/**
     ...
     */render:function render(updateLi){var that=this,notDisabled;//Update the LI to match the SELECT
if(updateLi!==false){this.$element.find("option").each(function(index){var $lis=that.findLis().eq(that.liObj[index]);that.setDisabled(index,this.disabled||this.parentNode.tagName==="OPTGROUP"&&this.parentNode.disabled,$lis);that.setSelected(index,this.selected,$lis)})}this.togglePlaceholder();this.tabIndex();var selectedItems=this.$element.find("option").map(function(){if(this.selected){if(that.options.hideDisabled&&(this.disabled||this.parentNode.tagName==="OPTGROUP"&&this.parentNode.disabled))return;var $this=$(this),icon=$this.data("icon")&&that.options.showIcon?"<i class=\""+that.options.iconBase+" "+$this.data("icon")+"\"></i> ":"",subtext;if(that.options.showSubtext&&$this.data("subtext")&&!that.multiple){subtext=" <small class=\"text-muted\">"+$this.data("subtext")+"</small>"}else{subtext=""}if(typeof $this.attr("title")!=="undefined"){return $this.attr("title")}else if($this.data("content")&&that.options.showContent){return $this.data("content").toString()}else{return icon+$this.html()+subtext}}}).toArray();//Fixes issue in IE10 occurring when no default option is selected and at least one option is disabled
...
// =====================
$(window).on("load.bs.select.data-api",function(){$(".selectpicker").each(function(){var $selectpicker=$(this);Plugin.call($selectpicker,$selectpicker.data())})})})(jQuery);$(function(){var gallery=$(".gallery-slider").lightSlider({item:1,controls:true,pager:false,prevHtml:"<i class=\"icon-left\"></i>",nextHtml:"<i class=\"icon-right\"></i>",adaptiveHeight:true,slideMargin:0});$(".js-change-slide").on("click","li",function(e){$(this).closest(".js-change-slide").find("li").removeClass("active");$(this).addClass("active");gallery.goToSlide($(this).index()+1)});$(".selectpicker").selectpicker()});